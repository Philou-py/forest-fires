<script lang="ts">
	...
	let canvasWidthInput: HTMLInputElement;
	let heightInput: HTMLInputElement;
	let canvasHeightInput: HTMLInputElement;
	...
	let cellHeight = canvasHeight / height;
	let cellWidth = canvasWidth / width;

	let fourCellWidth = 4 * cellWidth;
	let fourCanvasWidth = 4 * canvasWidth;
	...
	function resizeCanvas(event: SubmitEvent) {
		...
		const wantedHeight = Number(canvasHeightInput.value);
		const wantedWidth = Number(canvasWidthInput.value);

		cellHeight = Math.max(1, Math.floor(wantedHeight / height));
		cellWidth = Math.max(1, Math.floor(wantedWidth / width));
		fourCellWidth = 4 * cellWidth;
...
<section class="fireGrid">
	<form onsubmit={resizeCanvas} class="canvasControls">
		<label>
			Hauteur de la grille
			<input
				type="number"
				step="50"
				disabled={ongoingSimulation}
				value="800"
		...
		<label>
			...
			<input
				...
				disabled={ongoingSimulation}
				value="800"
				bind:this={canvasWidthInput}
			/>
		</label>

		<button disabled={ongoingSimulation}>Redimensionner</button>
	</form>

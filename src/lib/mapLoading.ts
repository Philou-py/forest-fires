...
function loadGridFromImg(
	board: DrawingBoard,
	url: string,
	mapping: ColourMapping | DensityMapping,
	type: 'vegFg' | 'vegBg' | 'density',
	pixelThickness?: number
): Promise<void> {
		...
		loadImage(PUBLIC_BASE_URL + url).then((img) => {
			...
			for (let i = 0; i < loadedData.length; i += 4) {
				...
				if (dist < 0.1) {
					...
					else if (type === 'density') board.grid[row][col].density = closestItem as DensityType;
				}
...
export async function loadForestWithDensity(
	width: number,
	height: number,
	canvasWidth: number,
	canvasHeight: number
